...
In-session, substitute the value of `qmd_index` directly in your commands (the index name is stable across the vault's lifetime). The default is declared by this vault manifest.
...
- Loading context for review prep → `mcp__qmd__multi_get` with `pattern="reviews/evidence/*.md"`.
