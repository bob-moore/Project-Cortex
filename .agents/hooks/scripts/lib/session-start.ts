...
/**
 * Format the "Active Work" section from project names or filenames in
 * Projects/active.
 ...
 * list — files like AGENTS.md, README.*.md, Home.md that aren't user content
 ...
 * patterns. Patterns are literal filenames (`AGENTS.md`) or globs with `*`
 ...
 * order — the caller decides which sources matter most (e.g. Projects/active
 ...
 * Output shape:
 *
 *   Projects/active/project-x/project-x.md
 *   - [ ] task one
 *   - [ ] task two
 *
 *   2026-05-18.md
...
/**
 * Format the "Harness Topics" section — one line per harness/ note with its
 * description from frontmatter, so the runtime sees what topic notes exist
 * without loading their full content. Omits the active operator/user/manual
 * files that are loaded directly elsewhere. Appends "(empty)" when the note
 * has no filled bullets, so the runtime knows not to waste a read on a stub.
 */
