 ...
 * (`.agents/hooks/scripts/qmd-refresh.ts`), the Stop hook
 * (`.agents/hooks/scripts/stop-checklist.ts`), and the detached worker
 * (`.agents/hooks/scripts/qmd-refresh-run.ts`). Centralizing the lifecycle
 ...
 * directory. Hook scripts live at `<vault>/.agents/hooks/scripts/`, so going
 * three segments up resolves the vault root irrespective of the invoking
