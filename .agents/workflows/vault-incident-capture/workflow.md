...
Capture an incident from Slack channels, DMs, and threads into structured vault notes with timeline, people, analysis, and evidence.

## Invocation

- Canonical workflow: `vault-incident-capture`
- Runtime adapters may expose this as `/vault-incident-capture`, `vault-incident-capture`, or another runtime-native trigger. The behavior belongs here.
...
Capture an incident from Slack channels, DMs, and threads into structured vault notes. Produces a complete incident work note with timeline, people, analysis, and evidence. Review evidence is optional and only written when requested or approved.

### Usage

```
/vault-incident-capture <slack-urls>
```
...
This command orchestrates two roles for heavy lifting:
...
Create `Notes/incidents/<Incident Name>.md` with:
...
- `Notes/Index.md` — list under the incidents bullet
- `harness/memory.md` — add incident summary to Recent Context
- `harness/patterns.md` — if this reveals a recurring pattern
- `harness/gotchas.md` — if this reveals a technical gotcha
- `reviews/Index.md` — update only if review evidence is requested or approved
- `reviews/cycles/YYYY-QN.md` — add candidate review evidence only when requested or approved
...
- Create `Notes/incidents/TICKET-XXXX Post-Mortem Draft.md` following your org's post-mortem template
