...
- Canonical workflow: `vault-meeting`
- Runtime adapters may expose this as `/vault-meeting`, `vault-meeting`, or another runtime-native trigger. The behavior belongs here.
...
Examples:
- `/vault-meeting Janus Project`
- `/vault-meeting Legal sign-off workflow`
- `/vault-meeting Qwen font exploration`
- `/vault-meeting ComfyUI model server access`
...
5. **Harnessstorm considerations** — go beyond what's in the vault; think through:
   ...
   - Things that have tripped up similar work before (check `harness/gotchas.md`)
   - Stakeholders who aren't in the room but whose work this affects

6. **Note relevant people** — list who's involved and their role relative to this topic only (not relationship history or 1:1 context — that's `/vault-prep-1on1`)
...
7. **Present the briefing**:
   ...
   Harnessstormed angles, risks, dependencies, and edge cases — especially things NOT already in the vault. This is the most valuable section.
...
Use `/vault-dump` to capture outcomes, decisions, and new context — it will route everything to the right notes.
...
- This is NOT for 1:1 prep — use `/vault-prep-1on1` for person-focused meeting prep
