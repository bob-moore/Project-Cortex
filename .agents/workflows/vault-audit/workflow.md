...
- `Home.md#Todo` — do task Base views resolve, and does each open reviewable artifact have a source-linked `review` task?
...
- Artifacts requiring User inspection should link to a `Tasks/` record with `status: review`, clear owner, next action, and acceptance criteria.
- Each open generated artifact should have a source-linked task with `status: review`.
- Do not close review tasks based on age alone; record the user decision, create downstream tasks only for real remaining work, and then close the review task.
