...
import { warn } from "../.agents/hooks/scripts/lib/hook-io.ts";
import { isMainModule } from "../.agents/hooks/scripts/lib/main-guard.ts";
import {
...
import {
	...
	translateToGlob,
	writeQmdIgnore,
} from "../.agents/hooks/scripts/lib/qmd-ignore.ts";
import { isValidQmdIndex } from "../.agents/hooks/scripts/lib/session-start.ts";

type ManifestSubset = {
	readonly qmd_index?: string;
	readonly qmd_context?: string;
	readonly qmd_collection?: string;
};
...
function main(): void {
	...
	// so a typo'd `qmd_collection` field can't land an unparseable identifier.
	// (`qmd_index` was validated above, so a failure here can only come from
	// `qmd_collection`.)
	if (!isValidQmdIndex(collectionName)) {
		process.stderr.write(
			`vault-manifest.json \`qmd_collection\` field value ${JSON.stringify(collectionName)} ` +
